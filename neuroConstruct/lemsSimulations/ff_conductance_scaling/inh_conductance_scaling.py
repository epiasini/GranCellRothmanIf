...
def main():
    ...
    inh_scaling_factor_range = np.arange(0.5, 1.6, 0.25)
    out_rates = np.zeros(shape=(inh_scaling_factor_range.size, exc_rate_range.size))
    sim_duration = 6 # s
    
    for i, inh_scaling_factor in enumerate(inh_scaling_factor_range):
        for k, exc_rate in enumerate(exc_rate_range):
            sim_data = simulation_tools.simulate_poisson_stimulation([exc_rate for each in range(n_stims)], sim_duration, inh_scaling_factor=inh_scaling_factor, exc_scaling_factor=1)

            spike_count = sim_data[-1,2]
            out_rates[i,k] = float(spike_count) / sim_duration
            print("inh: {}; input: {}Hz; output: {:.2f}Hz".format(inh_scaling_factor,
                                                                  exc_rate,
                                                                  out_rates[i,k]))
    ...
    c_norm  = colors.Normalize(vmin=inh_scaling_factor_range[0], vmax=inh_scaling_factor_range[-1])
    ...
    for i, inh_scaling_factor in enumerate(inh_scaling_factor_range):
        color = scalar_map.to_rgba(inh_scaling_factor)
